...
import { getJob, updateJob, getSkills, getCategories } from '../../api/jobs';
...
const STATUSES = ['active', 'draft', 'inactive', 'closed'];
...
const EditJob = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [jobRes, skillsRes, catsRes, compsRes] = await Promise.all([
          getJob(id),
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Failed to update job. Please check all fields and try again.'));
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!form) return <Alert severity="error" sx={{ m: 3 }}>{error || 'Job not found.'}</Alert>;
  ...
  return (
    <Box sx={{ maxWidth: 900, mx: 'auto', px: 2, py: 4 }}>
      ...
      <Typography variant="h4" fontWeight={700} mb={1}>Edit Job</Typography>
      ...
      <Box component="form" onSubmit={handleSubmit} noValidate>
        ...
        <Paper elevation={1} sx={{ p: 3, mb: 3 }}>
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} sm={8}>
              <TextField fullWidth required label="Job Title" name="title" value={form.title} onChange={handleChange} />
            </Grid>
            <Grid item xs={12} sm={4}>
              <TextField fullWidth label="Slug" name="slug" value={form.slug} onChange={handleChange} />
            </Grid>
            <Grid item xs={12}>
              <TextField fullWidth required multiline rows={5} label="Description" name="description" value={form.description} onChange={handleChange} />
            </Grid>
            <Grid item xs={12}>
              <TextField fullWidth multiline rows={4} label="Requirements" name="requirements" value={form.requirements} onChange={handleChange} />
            </Grid>
            <Grid item xs={12}>
              <TextField fullWidth multiline rows={4} label="Responsibilities" name="responsibilities" value={form.responsibilities} onChange={handleChange} />
            </Grid>
            <Grid item xs={12}>
              <TextField fullWidth multiline rows={3} label="Benefits" name="benefits" value={form.benefits} onChange={handleChange} />
            </Grid>
          </Grid>
        </Paper>
            ...
            <Grid item xs={12} sm={4}>
              <FormControl fullWidth required>
                ...
                <Select name="job_type" value={form.job_type} onChange={handleChange} label="Job Type">
                  {JOB_TYPES.map((t) => <MenuItem key={t} value={t}>{fmt(t)}</MenuItem>)}
                </Select>
              </FormControl>
            </Grid>
            <Grid item xs={12} sm={4}>
              <FormControl fullWidth required>
                ...
                <Select name="work_mode" value={form.work_mode} onChange={handleChange} label="Work Mode">
                  {WORK_MODES.map((m) => <MenuItem key={m} value={m}>{fmt(m)}</MenuItem>)}
                </Select>
              </FormControl>
            </Grid>
            <Grid item xs={12} sm={4}>
              <FormControl fullWidth>
                ...
                <Select name="experience_level" value={form.experience_level} onChange={handleChange} label="Experience Level">
                  ...
                  {EXPERIENCE_LEVELS.map((l) => <MenuItem key={l} value={l}>{fmt(l)}</MenuItem>)}
                </Select>
              </FormControl>
            </Grid>
            <Grid item xs={12} sm={4}>
              <FormControl fullWidth>
                ...
                <Select name="status" value={form.status} onChange={handleChange} label="Status">
                  {STATUSES.map((s) => <MenuItem key={s} value={s}>{fmt(s)}</MenuItem>)}
                </Select>
              </FormControl>
            </Grid>
            <Grid item xs={12} sm={4}>
        ...
        <Paper elevation={1} sx={{ p: 3, mb: 3 }}>
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={6} sm={4}>
              <TextField fullWidth label="Salary Min" name="salary_min" type="number" value={form.salary_min} onChange={handleChange} inputProps={{ min: 0 }} />
            </Grid>
            <Grid item xs={6} sm={4}>
              <TextField fullWidth label="Salary Max" name="salary_max" type="number" value={form.salary_max} onChange={handleChange} inputProps={{ min: 0 }} />
            </Grid>
        ...
        <Paper elevation={1} sx={{ p: 3, mb: 3 }}>
          ...
          <Autocomplete
            ...
            renderInput={(params) => (
              <TextField {...params} label="Select Skills" placeholder="Search skills..." />
            )}
          />
        </Paper>
